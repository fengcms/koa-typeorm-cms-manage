    ...
    <el-form ref="ruleForm" :model="form" :rules="rules" label-width="80px">
      <el-tabs v-model="pageInfo.tabActive">
        <el-tab-pane label="基础信息" name="base">
          ...
          <el-form-item label="文章类型" clearable prop="type">
            <el-radio-group v-model="form.type">
              <el-radio v-for="i in base.type" :key="i.value" :label="i.value">{{ i.label }}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="是否置顶" clearable prop="istop">
            <el-radio-group v-model="form.istop">
              <el-radio v-for="i in base.istop" :key="i.value" :label="i.value">{{ i.label }}</el-radio>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item label="文章视频" prop="video">
            <el-input v-model="form.video" clearable style="width: 400px;" />
            <VideoTool v-model="form.video" />
            <Tip block>可以从优酷、哔哩哔哩等视频网站获取视频地址</Tip>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        markdown: [{ required: true, message: '请输入文章内容', trigger: 'blur' }],
  ...
  methods: {
    ...
    afterGetData(data) {
      const channel = []
      const calcChan = (id) => {
        const chanItem = this.base.channel.find((i) => i.id === id)
        if (chanItem) {
